...
import { SupabaseAdminDb } from "./supabase-admin-db";
...
function getSupabaseAdminDb() {
  return new SupabaseAdminDb(
  ...
  if (token) {
    try {
      ...
      if (data.user) {
        ...
        tenantId = await getOrProvisionUser(
          supabase,
          data.user.id,
          data.user.email ?? "",
        );
