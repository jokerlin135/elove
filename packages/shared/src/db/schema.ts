...
export const plan_entitlements = pgTable(
  "plan_entitlements",
  {
    id: uuid("id").defaultRandom().primaryKey(),
  ...
  (t) => [index("pe_plan_idx").on(t.plan_id)],
);
...
export const published_versions = pgTable(
  "published_versions",
  {
    id: uuid("id").defaultRandom().primaryKey(),
  ...
  (t) => [index("pv_project_idx").on(t.project_id)],
);
...
export const media = pgTable(
  ...
  (t) => [
    index("media_tenant_idx").on(t.tenant_id),
    index("media_project_idx").on(t.project_id),
  ],
);

// ============ RSVP + GUESTBOOK ============

export const rsvp_responses = pgTable(
  "rsvp_responses",
  {
    id: uuid("id").defaultRandom().primaryKey(),
    project_id: uuid("project_id")
  ...
  (t) => [index("rsvp_project_idx").on(t.project_id)],
);
...
export const quota_usage = pgTable(
  ...
  (t) => [index("quota_tenant_idx").on(t.tenant_id)],
);
...
export const build_jobs = pgTable(
  "build_jobs",
  {
    id: uuid("id").defaultRandom().primaryKey(),
    project_id: uuid("project_id")
  ...
  (t) => [index("bj_status_queued_idx").on(t.status, t.queued_at)],
);
